...
class SettingsDialog(QDialog):
    ...
    def _create_steam_tab(self) -> None:
        ...
        self.sls_config_management_checkbox = create_checkbox_setting(
            ...
            True,
            self,
            f"Permite que o LumaTools gerencie os arquivos de configuração do {wrapper_name}.",
        )
    ...
    def _create_tools_tab(self) -> None:
        ...
        if sys.platform == "win32":
            ...
            SettingsDialog._add_tool_button(
                ...
                "Register lumatools:// URL protocol and .zip context menu entries.",
            ...
            SettingsDialog._add_tool_button(
                ...
                "Remove lumatools:// URL protocol and .zip context menu entries.",
    ...
    def _create_automation_tab(self) -> None:
        ...
        self.start_minimized_checkbox = create_checkbox_setting(
            ...
            True,
            self,
            "Inicia o LumaTools escondido na bandeja do sistema quando aberto com --start-hidden ou pelo autostart.",
        )
        ...
        self.close_to_tray_checkbox = create_checkbox_setting(
            ...
            True,
            self,
            "Ao clicar em fechar, mantém o LumaTools em segundo plano na bandeja.",
        )
        startup_layout.addWidget(self.close_to_tray_checkbox)

        self.auto_close_with_steam_checkbox = create_checkbox_setting(
            "Fechar LumaTools quando a Steam fechar",
            "auto_close_with_steam",
            False,
            self,
            "Monitora a Steam e encerra o LumaTools automaticamente para economizar RAM.",
        )
        ...
        hint = QLabel(
            "Modo stealth usa a bandeja do sistema. Se seu ambiente não expor tray, o LumaTools apenas iniciará minimizado."
        )
    ...
    def _create_opencloudsave_tab(self) -> None:
        ...
        self.opencloudsave_remote_input.setPlaceholderText(
            "Exemplo: meudrive:LumaTools-Saves"
        )
        ...
        hint = QLabel(
            "Os caminhos de save por jogo são configurados na Biblioteca do LumaTools, dentro da aba OpenCloudSave de cada jogo."
        )
        hint.setWordWrap(True)
        hint.setStyleSheet("color: #A6A6A6; font-size: 13px;")
        layout.addWidget(hint)
    ...
    def _create_discord_tab(self) -> None:
        ...
        self.discord_presence_checkbox = create_checkbox_setting(
            "Ativar Rich Presence do LumaTools",
        ...
        SettingsDialog._add_tool_explanation(
            rpc_layout,
            "O Rich Presence já vem ligado por padrão. Se o Discord estiver aberto e esta build tiver um Client ID válido, o LumaTools aparece sozinho.",
        )
        ...
        self.discord_small_image_input.setPlaceholderText(
            "Opcional: deixe em branco para o LumaTools alternar por estado"
        )
        ...
        SettingsDialog._add_tool_explanation(
            advanced_layout,
            "O Discord não aceita GIF bruto no Rich Presence. O LumaTools usa imagens estáticas por estado e você pode sobrescrever os assets aqui se mantiver uma aplicação própria.",
        )
    ...
    def _create_updates_tab(self) -> None:
        ...
        self.github_updates_checkbox = create_checkbox_setting(
            ...
            True,
            self,
            "Consulta o repositório no GitHub quando o LumaTools inicia.",
        )
    ...
    def _create_audio_tab(self) -> None:
        ...
        toggles_group = QGroupBox("Efeitos de áudio")
        ...
        self.play_etw_checkbox = create_checkbox_setting(
            ...
            True,
            self,
            "Toca o efeito ETW ao abrir o launcher.",
        )
        toggles_layout.addWidget(self.play_etw_checkbox)
        ...
        self.play_lall_checkbox = create_checkbox_setting(
            ...
            True,
            self,
            "Toca o efeito LALL ao fechar o launcher.",
        )
        toggles_layout.addWidget(self.play_lall_checkbox)
        ...
        self.play_50hz_hum_checkbox = create_checkbox_setting(
            ...
            True,
            self,
            "Mantém o hum ambiente ativo enquanto o app estiver aberto.",
        )
        toggles_layout.addWidget(self.play_50hz_hum_checkbox)

        preview_buttons_layout = QHBoxLayout()
        self.test_etw_button = QPushButton("Testar abertura")
        self.test_etw_button.clicked.connect(lambda: self._play_preview_sound("etw"))
        preview_buttons_layout.addWidget(self.test_etw_button)

        self.test_lall_button = QPushButton("Testar fechamento")
        self.test_lall_button.clicked.connect(lambda: self._play_preview_sound("lall"))
        preview_buttons_layout.addWidget(self.test_lall_button)
        toggles_layout.addLayout(preview_buttons_layout)

        toggles_group.setLayout(toggles_layout)
        layout.addWidget(toggles_group)

        volume_group = QGroupBox("Volumes")
        volume_layout = QVBoxLayout()

        master_layout, self.master_volume_slider, _, _ = create_slider_setting(
            "Master", "master_volume", 80, self
        )
        volume_layout.addLayout(master_layout)

        effects_layout, self.effects_volume_slider, _, _ = create_slider_setting(
            "Efeitos", "effects_volume", 50, self
        )
        volume_layout.addLayout(effects_layout)

        hum_layout, self.hum_volume_slider, _, _ = create_slider_setting(
            "Loop", "hum_volume", 20, self
        )
    ...
    def _preview_audio_mix(self) -> None:
        ...
        if not all(
            ...
            return
        self.main_window.audio_manager.preview_volume(
            self.master_volume_slider.value(),
            self.effects_volume_slider.value(),
            self.hum_volume_slider.value(),
        )

    def on_master_volume_changed(self, _value: int) -> None:
        self._preview_audio_mix()

    def on_effects_volume_changed(self, _value: int) -> None:
        self._preview_audio_mix()

    def on_hum_volume_changed(self, _value: int) -> None:
        self._preview_audio_mix()

    def _play_preview_sound(self, sound_key: str) -> None:
        if not self.main_window or not hasattr(self.main_window, "audio_manager"):
            return
        self._preview_audio_mix()
        audio_manager = self.main_window.audio_manager
        sound_type = {
            "etw": getattr(audio_manager, "SOUND_ETW", "etw"),
            "lall": getattr(audio_manager, "SOUND_LALL", "lall"),
        }.get(sound_key)
        if sound_type is None:
            return
        try:
            audio_manager.play_sound(sound_type, preview=True, block=False)
        except Exception as exc:
            logger.debug("Falha ao reproduzir preview de áudio: %s", exc)
    ...
    def accept(self) -> None:
        ...
        # Defer potentially expensive reconnect/reconfigure work until after the
        # dialog closes so saving settings never blocks the UI.
        if self.main_window and hasattr(self.main_window, "discord_presence_manager"):
            QTimer.singleShot(0, self.main_window.discord_presence_manager.reload_settings)
            QTimer.singleShot(50, self._refresh_discord_status)
    ...
    def _save_audio_settings(self) -> None:
        if not all(
            (
                self.play_etw_checkbox,
                self.play_lall_checkbox,
                self.play_50hz_hum_checkbox,
                self.master_volume_slider,
                self.effects_volume_slider,
                self.hum_volume_slider,
            )
        ):
            logger.debug("Audio widgets are not fully initialized; skipping audio save.")
            return
    ...
    def test_rclone_setup(self) -> None:
        ...
        if not remote:
            QMessageBox.warning(
                self,
                "Remoto ausente",
                "Defina um remoto rclone, por exemplo: meudrive:LumaTools-Saves",
            )
            return
    ...
    def _rollback_latest_backup(self) -> None:
        ...
        reply = QMessageBox.question(
            self,
            "Rollback",
            "O LumaTools será fechado para restaurar o backup mais recente. Continuar?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
    ...
    def register_registry_entries() -> None:
        SettingsDialog._manage_registry("LumaTools.reg", "Registrado com sucesso")

    @staticmethod
    def remove_registry_entries() -> None:
        SettingsDialog._manage_registry("LumaTools_uninstall.reg", "Removido com sucesso")
