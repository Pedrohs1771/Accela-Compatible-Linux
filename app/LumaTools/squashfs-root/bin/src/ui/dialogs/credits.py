...
class CreditsDialog(QDialog):
    ...
    def _create_credits_content(self):
        ...
        # Contact information
        address_label = QLabel("Discord: violeiro_bad")
        address_label.setStyleSheet("font-size: 12px; margin-top: 10px;")
        credits_info_layout.addWidget(address_label)

        # Project information
        phone_label = QLabel("Versão modificada em pt-BR do LumaTools para Linux")
        phone_label.setStyleSheet("font-size: 12px; margin-top: 5px;")
        credits_info_layout.addWidget(phone_label)

        upstream_label = QLabel(
            "Projeto original: LumaTools / enter-the-wired\n"
            "Crédito ao projeto original e ao trabalho open source disponibilizado por ciscosweater."
        )
