#!/usr/bin/env python3
"""LumaTools Steam UI injector utility."""

import os
import shutil
import subprocess

MARKER_START = "<!-- LumaTools Inject START -->"
MARKER_END = "<!-- LumaTools Inject END -->"
SCRIPT_TAG = '<script src="LumaTools/lumatools.js"></script>'
...
def _sync_assets(public_dir: str, target_dir: str):
    os.makedirs(target_dir, exist_ok=True)
    js_src = os.path.join(public_dir, "lumatools.js")
    if os.path.isfile(js_src):
        shutil.copy2(js_src, os.path.join(target_dir, "lumatools.js"))

    icon_src = os.path.join(public_dir, "lumatools-icon.png")
    if os.path.isfile(icon_src):
        shutil.copy2(icon_src, os.path.join(target_dir, "lumatools-icon.png"))
...
def ensure_ui_injection(install_root: str):
    public_dir = os.path.join(install_root, "public")
    result = {"roots_seen": 0, "roots_patched": 0, "assets_synced": 0}
    script_tag = _build_inline_script_tag(os.path.join(public_dir, "lumatools.js")) or SCRIPT_TAG
    ...
    for root in _candidate_steam_roots():
        ...
        result["roots_seen"] += 1
        target_dir = os.path.join(steamui, "LumaTools")
...
def main():
    ...
    print(f"roots_seen={stats['roots_seen']} roots_patched={stats['roots_patched']} assets_synced={stats['assets_synced']}")
    if stats["roots_seen"] == 0:
        print("Steam UI not found. Open Steam and try again.")
        return 1
    print("Injeção do LumaTools concluída com sucesso!")
    return 0
