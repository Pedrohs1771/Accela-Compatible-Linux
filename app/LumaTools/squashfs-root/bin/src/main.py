...
def set_app_user_model_id():
    ...
    if sys.platform == "win32":
        try:
            my_app_id = "god.is.in.the.wired.lumatools"
...
def _instance_server_name() -> str:
    if os.name == "posix":
        return f"lumatools.instance.{os.getuid()}"
    return "lumatools.instance"
...
def _cleanup_duplicate_gui_instances(logger) -> None:
    ...
    for process in duplicate_processes:
        try:
            logger.warning(
                f"Terminating duplicate LumaTools instance PID {process.pid}"
            )
            process.terminate()
...
class SingleInstanceCoordinator:
    ...
    def _process_connection_payload(self, socket: QLocalSocket) -> None:
        ...
        try:
            ...
            self.logger.info("Forwarded request received by running LumaTools instance")
            QTimer.singleShot(
...
def main():
    ...
    logger.info("========================================")
    logger.info(f"LumaTools {app_version} starting...")
    logger.info("========================================")
    ...
    def _parse_url_action(url: str):
        ...
        try:
            url_content = url[9:]  # Remove 'lumatools://'
            ...
            if action == "download" and param_val and param_val.isdigit():
                ...
                logger.info(
                    f"Found lumatools://{action} URL for AppID: {param_val} ({mode_str} mode)"
                )
            elif action == "zip" and param_val:
                _add_zip_from_url(param_val, "(GUI mode)" if not cli_mode else "")
            else:
                logger.warning(f"Invalid lumatools:// URL format: {url}")
    ...
    if instance_coordinator.forward_to_primary(instance_payload):
        logger.info("Existing LumaTools instance detected; forwarded request and exiting.")
        return None
    ...
    try:
        ...
        if config_path.exists():
            from utils.yaml_config_manager import ensure_advanced_bypasses
            if ensure_slssteam_api_enabled(config_path):
                logger.info("SLSsteam API enabled in config")
            if ensure_advanced_bypasses(config_path):
                logger.info("Advanced DLC/Workshop bypasses enabled in config")
    except OSError as e:
    ...
    try:
        ...
        exit_code = app.exec()
        try:
            if hasattr(main_win, "_shutdown_components"):
                main_win._shutdown_components()
        except Exception as cleanup_exc:
            logger.debug(f"Error during post-event-loop shutdown: {cleanup_exc}")
        try:
            TaskRunner.stop_all_active()
        except Exception as cleanup_exc:
            logger.debug(f"Error stopping TaskRunners after event loop: {cleanup_exc}")
        sys.exit(exit_code)
