"""
CLI Manager for LumaTools - Handles command-line mode for ZIP file processing.

When LumaTools is invoked with ZIP file arguments, this module takes over
...
def _get_terminal_command(
    ...
    # Get the directory where lumatools is installed
...
def open_cli_terminal(
    ...
    """Open a new terminal running LumaTools CLI mode.
...
def run_cli_mode(
    ...
    """Run LumaTools in CLI mode - show only DepotSelectionDialog for ZIP files.
    ...
    logger.info("=" * 50)
    logger.info("LumaTools CLI Mode")
    logger.info("=" * 50)

    # Fix GreenLuma offline mode
    fix_greenluma_offline_mode()

    # Apply LumaTools theme
