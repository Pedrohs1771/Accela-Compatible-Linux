...
class UpdateManager(QObject):
    ...
    def __init__(self, main_window):
        ...
        self._next_allowed_check_at = 0.0
        self.get_repo_slug()
    ...
    def cleanup(self) -> None:
        """Stop timers so shutdown doesn't leave live Qt objects behind."""
        for timer_name in (
            "_prepare_timer",
            "_heartbeat_timer",
            "_pending_auto_update_timer",
        ):
    ...
    def get_repo_slug(self) -> str:
        ...
        if configured != normalized:
            self._persist_repo_slug(normalized)
    ...
    def check_for_updates_async(self, interactive: bool = False, force: bool = False) -> None:
        ...
        if (
            not force
            and not interactive
            and self._next_allowed_check_at > time.time()
        ):
            return
    ...
    def _monitor_prepared_update(self, status_dir: Path) -> None:
        ...
        def poll() -> None:
            if ready_file.exists():
                self._prepare_timer.stop()
                self._set_status("Update validado. Fechando LumaTools para aplicar a troca.")
    ...
    def _check_worker(self, interactive: bool) -> None:
        try:
            ...
            self._apply_update_check_backoff(exc)
    ...
    def _apply_update_check_backoff(self, exc: Exception) -> None:
        response = getattr(exc, "response", None)
        if response is None:
            return

        if response.status_code == 403:
            reset_header = str(response.headers.get("X-RateLimit-Reset", "")).strip()
            reset_at = 0.0
            if reset_header.isdigit():
                reset_at = float(reset_header)
            if reset_at <= time.time():
                reset_at = time.time() + self.RATE_LIMIT_BACKOFF_SECONDS
            self._next_allowed_check_at = reset_at
            logger.warning(
                "GitHub rate limit hit; delaying next update check until %s",
                time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(reset_at)),
            )
            return

        if response.status_code == 404:
            self._next_allowed_check_at = time.time() + 300

    def _persist_repo_slug(self, repo: str) -> None:
        try:
            current = self.settings.value("github_updates_repo", "", type=str).strip()
            if current != repo:
                self.settings.setValue("github_updates_repo", repo)
                self.settings.sync()
        except Exception:
            logger.debug("Failed to persist sanitized GitHub repo slug", exc_info=True)
        ...
        if response.status_code == 200:
            ...
            if status in {"identical", "ahead"} or ahead_by > 0:
                return {
                    "available": False,
                    "label": installed_revision[:8],
                    "message": f"Seu LumaTools local já está adiantado em relação ao GitHub ({installed_revision[:8]}).",
                }

            return {
                "available": False,
                "label": installed_revision[:8],
                "message": "Seu LumaTools local divergiu do GitHub. Update automático foi preservado para evitar downgrade.",
            }
    ...
    def _fetch_latest_release(self) -> Dict[str, str]:
        ...
        try:
            ...
            if (
                ...
                and repo != self.DEFAULT_REPO
            ):
                logger.warning(
                    "Configured GitHub repo '%s' returned 404; falling back to default repo '%s'.",
                    repo,
                    self.DEFAULT_REPO,
                )
                self._persist_repo_slug(self.DEFAULT_REPO)
...
healthcheck() {{
    ...
    [ -f "$source_dir/app/LumaTools/squashfs-root/AppRun" ] || fail_update "Pacote sem AppRun."
    [ -f "$source_dir/app/LumaTools/squashfs-root/bin/run.sh" ] || fail_update "Pacote sem run.sh."
    bash -n "$source_dir/install.sh" || fail_update "install.sh inválido."
    bash -n "$source_dir/app/LumaTools/squashfs-root/AppRun" || fail_update "AppRun inválido."
    bash -n "$source_dir/app/LumaTools/squashfs-root/bin/run.sh" || fail_update "run.sh inválido."
    python3 -m compileall "$source_dir/app/LumaTools/squashfs-root/bin/src" >/dev/null || fail_update "Código Python do update não compila."
    if command -v desktop-file-validate >/dev/null 2>&1; then
        desktop-file-validate "$source_dir/app/LumaTools/squashfs-root/LumaTools.desktop" >/dev/null 2>&1 || fail_update ".desktop do update é inválido."
    fi
}}
