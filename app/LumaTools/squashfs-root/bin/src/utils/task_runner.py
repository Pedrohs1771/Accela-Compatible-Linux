...
class TaskRunner(QObject):
    ...
    def run(self, target_func, *args, **kwargs):
        ...
        if self._thread is not None and self._thread.isRunning():
            logger.debug(
                "TaskRunner already has an active thread for '%s'; stopping it first.",
                getattr(getattr(self.worker, "target_func", None), "__name__", "unknown"),
            )
            self.stop(wait_ms=1000, terminate_on_timeout=True)

        TaskRunner._runner_counter += 1
        thread_name = f"TaskRunner:{target_func.__name__}:{TaskRunner._runner_counter}"

        # Keep the worker thread independent from QObject parent destruction so
        # shutdown can stop it explicitly before Qt tears down the object tree.
        ...
        if self._thread is not None and self._thread.isRunning():
            try:
                ...
                if wait_ms > 0 and not self._thread.wait(wait_ms):
                    logger.warning(
                        "Thread '%s' did not finish in time during stop()",
                        self._thread.objectName(),
                    )
            ...
            try:
                ...
                logger.debug("Thread was already deleted while waiting after stop()")
        self._thread = None
        self.worker = None
    ...
    def stop_all_active(cls):
        ...
        for runner in runners:
            try:
                runner.stop(wait_ms=1000, terminate_on_timeout=True)
            except RuntimeError as e:
