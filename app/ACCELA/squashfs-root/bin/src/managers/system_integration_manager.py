...
class SystemIntegrationManager(QObject):
    ...
    def get_autostart_path() -> Path:
        return Path.home() / ".config" / "autostart" / "accela.desktop"

    def _build_autostart_desktop_entry(self) -> str:
        launcher_path = Path.home() / ".local" / "bin" / "accela"
        if not launcher_path.exists():
            launcher_path = Path.home() / ".local" / "share" / "ACCELA" / "ACCELA.AppImage"
    ...
    def _poll_steam_state(self) -> None:
        ...
        if self._steam_was_running and not steam_running:
            logger.info("Steam was closed; notifying ACCELA for shutdown handling.")
